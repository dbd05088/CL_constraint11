...
class MemoryDataset(Dataset):
    ...
    def get_ratio(self, y):
        ...
        real_count = 0
        for cls in y:
            if self.usage_cnt[cls] > self.klass_train_warmup: # 초반 부분에는 Loss가 너무 클 수 있기 때문에
                batch_loss += self.cls_loss[cls].item()
                real_count += 1
    ...
    @torch.no_grad()
    def get_batch(self, batch_size, stream_batch_size=0, use_weight=None, transform=None, twf=False, recent_ratio=None, exp_weight=False, prev_batch_index=None, weight_method=None, n_class=None, avg_prob=None, class_cutmix=False):
...
class ASERMemory(MemoryDataset):
    ...
    def get_aser_calculate_batches(self, n_smp_cls, candidate_size):
        ...
        if len(self.images) > 0:  
            ...
            eval_indices = self.get_class_balance_indices(n_smp_cls, candidate_size=None, discard_indices = candidate_indices)
            images = []
            labels = []
            for i in eval_indices:
                images.append(self.test_transform(self.images[i]))
